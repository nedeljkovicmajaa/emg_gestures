...
for sub in range(9, 41):
    path1 = f"../rad/Hampel signals/hampel_S{sub}_E1.npy"
    path = f"../DB2/DB2_s{sub}/S{sub}_E1_A1.mat"
    # Učitavanje signala
    mat = si.loadmat(path)
    emg_signal = np.transpose(np.load(path1))
    ...
    while j < len(labele[0]) and broj_pokreta < 102:
        ...
        while br < 16:
            ...
            img_path = f"../rad/Windowing Hampel/window_hampel_S{sub}_E{sub}_Mov{mov}_Pon{pon}_Sli{br+1}.npy"
